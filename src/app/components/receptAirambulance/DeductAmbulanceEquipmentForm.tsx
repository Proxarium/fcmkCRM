// components/DeductAmbulanceEquipmentForm.tsx
"use client"
...
interface DeductAmbulanceEquipmentFormProps {
  ambulanceId: string;
  equipment: {
    id: string;
    name: string;
  ...
  userId: string;
  saveDeduction: (data: { callCardNumber: string; ambulanceId: string; deductedItems: DeductItem[]; userId: string }) => Promise<any>;
}

export default function DeductAmbulanceEquipmentForm({ ambulanceId, equipment, userId, saveDeduction }: DeductAmbulanceEquipmentFormProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setToastMessage('Рецепт отправлен'); 
      router.refresh();
    } catch (error) {
